...
Epha is a Kubernetes Operator to maintain annotations across targets in a cluster for application specific purposes.
...
```sh
make docker-build docker-push IMG=<some-registry>/epha:tag
```
...
```sh
make deploy IMG=<some-registry>/epha:tag
```
